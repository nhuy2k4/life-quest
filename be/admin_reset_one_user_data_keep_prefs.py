...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reset one user's data while keeping the users row and user_preferences.",
    )
...
async def reset_user_data(identifier: str, *, keep_sessions: bool) -> None:
    async with AsyncSessionLocal() as session:
        ...
        print("[KEEP] users row and user_preferences will be preserved.")
        ...
        # Gameplay
        await session.execute(delete(Submission).where(Submission.id.in_(submission_ids)))
        await session.execute(delete(UserQuest).where(UserQuest.user_id == user.id))

        # Notifications + achievements + recommendations
        ...
        print("[RESET] quests, submissions, XP, social data, recommendations, notifications, badges.")
        print("[KEPT] users row, user_preferences.")
        if keep_sessions:
