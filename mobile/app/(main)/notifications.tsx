...
type NotifType = 'like' | 'comment' | 'follow' | 'quest_complete' | 'quest_rejected' | 'quest_suggest' | 'xp' | 'system';
...
function notificationCopy(item: NotificationItem): { actor?: string; preview: string; meta?: string } {
  ...
  if (type === 'quest_rejected') {
    return {
      preview: 'Quest photo rejected',
      meta: readString(item.data, 'reason') ?? 'You can update the photo and try again.',
    };
  }
...
function NotificationRow({
  ...
  const type = asNotifType(item.type);
  const isSystem = type === 'quest_complete' || type === 'quest_rejected' || type === 'quest_suggest' || type === 'xp' || type === 'system';
  const iconBg = type === 'quest_complete' ? '#11181C' : type === 'quest_rejected' ? '#7F1D1D' : '#1F2937';
